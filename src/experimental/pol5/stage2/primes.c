...
prime_table_init()
{
	...
	for (ind = 0; ind < 11; ind++)
		table_small_primes[ind] = pr[ind];
	memset(tmp, 1, SMALLPRIMELIMIT);
	tmp[1] = 0;
	for (i = 0; i < 11; i++) {
		p = (int) (pr[i]);
		x = 0;
		while (x < SMALLPRIMELIMIT) {
			tmp[x] = 0;
			x += p;
		}
	}
...
void
set_prime_table(unsigned int p)
{
...
get_next_prime()
{
	...
	while (1) {
		while (prime_ptr < table_ptr_end) {
			if (*prime_ptr)
				return (1 + table_displacement +
					2 * (prime_ptr - table_primes));
			prime_ptr++;
		}
		compute_prime_table();
	}
