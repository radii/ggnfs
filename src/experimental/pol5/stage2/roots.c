...
static void
init_find_roots(int deg, double *polcoeff)
{
	int i, j;
	...
	for (i = 0; i < deg; i++)
		roots_pol_deriv[i] =
			(double *) xmalloc((deg + 1) * sizeof(double));
	for (j = 0; j <= deg; j++)
		roots_pol_deriv[0][j] = polcoeff[j];
	for (i = 1; i < deg; i++) {
		for (j = 0; j < deg; j++)
			roots_pol_deriv[i][j] =
				roots_pol_deriv[i - 1][j + 1] * (double) (j +
									  1);
...
exit_find_roots()
{
	...
	for (i = 0; i < roots_deg; i++)
		free(roots_pol_deriv[i]);
...
static double
roots_eval(int deriv, double x)
{
	double res, xn;
	int i;

	res = roots_pol_deriv[deriv][0];
	xn = 1.;
...
static int
roots_find(int deriv, double d0, double d1, double *dr)
{
	double p0, p1, db, de, pm;
	volatile double dm;	/* important for some compilers */

	p0 = roots_eval(deriv, d0);
	p1 = roots_eval(deriv, d1);
	if (p1 == 0.) {
		*dr = d1;
		return 1;
	}
...
int
find_roots(int deg, double *polcoeff, double **op)
{
	...
	roots_derivroots[0] = DBL_MIN;
	roots_derivroots[1] = DBL_MAX;
	n = 0;
	for (deriv = deg - 1; deriv >= 0; deriv--) {
		j = 1;
		d = roots_derivroots[0];
		for (i = 1; i < n + 2; i++) {
			dd = roots_derivroots[i];
			if (roots_find(deriv, d, dd, roots_derivroots + j))
				j++;
			d = dd;
		}
		if (roots_derivroots[j - 1] < DBL_MAX) {
			roots_derivroots[j] = DBL_MAX;
			j++;
		}
		n = j - 2;
	}
	for (i = 0; i < n; i++)
		roots_optima[i] = roots_derivroots[i + 1];
	nroots = n;
	...
	while (i < nroots - 1) {
		if (roots_optima[i] == roots_optima[i + 1]) {
			for (j = i + 1; j < nroots - 1; j++)
				roots_optima[j] = roots_optima[j + 1];
			nroots--;
			continue;
		}
		if (roots_optima[i] > roots_optima[i + 1]) {
			d = roots_optima[i];
			roots_optima[i] = roots_optima[i + 1];
			roots_optima[i + 1] = d;
			if (i > 0)
				i--;
			continue;
...
int
find_optima(int *deg, double **coeff, double skewness, double **optima)
{
	int n, nop, s;
	double *op, *res;
