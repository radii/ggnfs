...
define(ot_tester1,n_i_reg)dnl
define(ot_tester2,0)dnl
function_head(lasched2)
...
lasched2_ij_loop:
	...
	and li=ij,n_i_mask_reg
	shladd sched_elem=sched_index,3,sched;;
	(p6)br.dptk lasched2_ij_loop
lasched2_ij_loop_ende:
	sub ij=ij,ij_ub
	add fbi_offs=fbi_offs,reg65536;;
	st4 [ij_ptr]=ij,4
	br.ctop.sptk lasched2_fbi_loop;;
	add ri_ptr=-8,ri_ptr
	mov ar.pfs=r31
	br.ret.sptk b0
...
ot2_exception:
	sub aux1=ri1,ri0
	cmp.leu p8,p9=ri0,ri1
	cmp.eq p10,p11=la,lb ;;
	shladd sched_elem=sched_index,3,sched
	(p8)mov ij=n_i
	(p9)br.spnt ot2_ok;;
	(p10)shr.u ij=aux1,1
