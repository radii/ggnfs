...
static u16_t mpqs_nrels_active, mpqs_nlp_active;
static u16_t mpqs_lp_count[128*16+MPQS_MAX_FBSIZE+3+MPQS_MAX_ADIV_ALL];
...
void mpqs_init()
{
  ...
  mpz_init(mpqs_sq1);
  mpz_init(mpqs_sq2);
  mpqs_FB_start=xmalloc(2*MPQS_MAX_FBSIZE*sizeof(*mpqs_FB_start));
  mpqs_FB_inv_info=xmalloc(4*MPQS_MAX_NPRIMES*sizeof(*mpqs_FB_inv_info));
...
#else
      p=fb[2*j]; /*s1=fbs[2*j]; s2=fbs[2*j+1];*/
      aa=mpqs_SI_add[i][j]; al=(((u32_t)aa)<<16)+(u32_t)aa;
      ul=(u32_t *)(fbs+2*j);
#define GCC_BUG
#ifndef GCC_BUG
      *ul+=al;
...
#else
      p=fb[2*j]; /*s1=fbs[2*j]; s2=fbs[2*j+1];*/
      aa=p-mpqs_SI_add[i][j]; al=(((u32_t)aa)<<16)+(u32_t)aa;
      ul=(u32_t *)(fbs+2*j);
#ifndef GCC_BUG
      *ul+=al;
#else
...
#if 1
  asm_mpqs_sieve(mpqs_sievearray,mpqs_sievelen,fb,fbs,fblog+i,pb);
#else
  for (;;) {
    p=*fb; if (p>pb) break;
...
{
#if defined( EVAL8 ) && !defined(ASM_MPQS_EVAL)
  u64_t *ullsv,*ullsvend,h;
#else
  ...
  ullsvend=ullsv+(mpqs_sievelen>>3);
#else
  ulsv=(u32_t *)mpqs_sievearray;
  ...
  while (ullsv<ullsvend) {
    h=*ullsv;
    if (h&0x8080808080808080) {
      sv=(uchar *)ullsv;
      for (i=0; i<8; i++)
        if (*sv++&0x80)
          buffer[mpqs_nsurvivors++]=(u16_t)(sv-mpqs_sievearray-1);
      if (mpqs_nsurvivors>nmaxsurv-8) {
        while (ullsv<ullsvend) *ullsv++=0;
        break;
      }
    }
    *ullsv++=0;
...
static __INLINE__ u16_t mpqs_hash_lp(u32_t lp)
{
  long j;
  u32_t lp1;
  u16_t lphash, nh;
...
static __INLINE__ int mpqs_hash_rel(i64_t axb)
{
  long j;
  u32_t hash, hash2, nh;
...
static void mpqs_decompose()
{
  ...
  u16_t ind, p, s1, s2, nr, d, lpi, r, ii;
  u16_t **rels;
  i64_t axb, *llp;
  u64_t qx, pr, prasm;
  u16_t d2, ii2, minus;
