...
void gcd64(ulong *gcd, ulong *a, ulong *b)
{
  ...
  if (!(a[0]|a[1])) { gcd[0]=b[0]; gcd[1]=b[1]; return; }
  if (!(b[0]|b[1])) { gcd[0]=a[0]; gcd[1]=a[1]; return; }
  bb[0]=b[0]; bb[1]=b[1]; aa[0]=a[0]; aa[1]=a[1];
  while (!(bb[0]&1)) {
    bb[0]=(bb[0]>>1)|(bb[1]<<31); bb[1]>>=1;
  }
  while (!(aa[0]&1)) {
    aa[0]=(aa[0]>>1)|(aa[1]<<31); aa[1]>>=1;
  }
  while (1) {
    asm_diff64(aa,bb,r);
    if (!(r[0]|r[1])) break;
    while (!(r[0]&1)) {
      r[0]=(r[0]>>1)|(r[1]<<31); r[1]>>=1;
    }
...
void gcd(ulong *gcd, ulong *a, ulong *b)
{
  ...
  while (!(bb[0]&1)) {
    for (i=0; i<montgomery_ulongs-1; i++) bb[i]=(bb[i]>>1)|(bb[i+1]<<31);
    bb[montgomery_ulongs-1]>>=1;
  }
  while (!(aa[0]&1)) {
    for (i=0; i<montgomery_ulongs-1; i++) aa[i]=(aa[i]>>1)|(aa[i+1]<<31);
  ...
  while (1) {
    ...
    while (!(r[0]&1)) {
      for (i=0; i<montgomery_ulongs-1; i++) r[i]=(r[i]>>1)|(r[i+1]<<31);
...
void asm_half_old(ulong *a)
{
  ...
  for (i=0; i<montgomery_ulongs-1; i++)
    n_half[i]=(montgomery_modulo_n[i]>>1)|(montgomery_modulo_n[i+1]<<31);
  n_half[montgomery_ulongs-1]=montgomery_modulo_n[montgomery_ulongs-1]>>1;
  asm_add2_ui(n_half,1);  /* (N+1)/2 */

  c=a[0]&1;
  for (i=0; i<montgomery_ulongs-1; i++) a[i]=(a[i]>>1)|(a[i+1]<<31);
...
int asm_invert(ulong *res, ulong *b)  /* inverts b mod N */
{
  ...
  while (1) {
    ...
    if (t1[i]>v1[i]) {  /* t1>v1 */
      ...
      asm_sub(v2,v2,t2);
      do {
        for (i=0; i<len; i++) v1[i]=(v1[i]>>1)|(v1[i+1]<<31);
        v1[len]>>=1;
        asm_half(v2);
      } while (!(v1[0]&1));
    }
