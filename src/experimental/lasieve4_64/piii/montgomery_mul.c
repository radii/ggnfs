...
void init_montgomery_R2_2()
{
  ...
  h[0]=1; h[1]=0;
  for (i=0; i<128; i++) {
    if (h[1]&0x80000000) c=1; else c=0;
    h[1]=(h[1]<<1)|(h[0]>>31);
    h[0]<<=1;
    if (!c) {
      if (h[1]<montgomery_modulo_n[1]) continue;
      if (h[1]==montgomery_modulo_n[1]) {
        if (h[0]<montgomery_modulo_n[0]) continue;
      }
    }
    if (h[0]<montgomery_modulo_n[0]) c=1; else c=0;
...
void init_montgomery_R2_3()
{
  ...
  h[0]=1; h[1]=0; h[2]=0;
  for (i=0; i<192; i++) {
    if (h[2]&0x80000000) c=1; else c=0;
    h[2]=(h[2]<<1)|(h[1]>>31);
    h[1]=(h[1]<<1)|(h[0]>>31);
    h[0]<<=1;
    if (!c) {
      if (h[2]<montgomery_modulo_n[2]) continue;
    ...
    if (h[1]<montgomery_modulo_n[1]) c=1; else c=0;
    h[1]-=montgomery_modulo_n[1];
...
void init_montgomery_R2()
{
  ...
  h[0]=1; for (i=1; i<montgomery_ulongs; i++) h[i]=0;
  for (i=0; i<64*montgomery_ulongs; i++) {
    if (h[montgomery_ulongs-1]&0x80000000) c=1; else c=0;
    for (j=montgomery_ulongs-1; j>0; j--) h[j]=(h[j]<<1)|(h[j-1]>>31);
    h[0]<<=1;
    if (!c) {
      for (j=montgomery_ulongs-1; j>=0; j--)
        if (h[j]!=montgomery_modulo_n[j]) break;
      if ((j>=0) && (h[j]<montgomery_modulo_n[j])) continue;
    ...
    for (j=0; j<montgomery_ulongs; j++) {
      if (c) {
        if (montgomery_modulo_n[j]+1==0) c=1;
        else {
          if (h[j]<(montgomery_modulo_n[j]+1)) c=1; else c=0;
          h[j]-=(montgomery_modulo_n[j]+1);
        }
      } else {
        if (h[j]<montgomery_modulo_n[j]) c=1; else c=0;
        h[j]-=montgomery_modulo_n[j];
      }
    }
    if (c) complain("init_montgomery_R2\n");
#endif
  }
  for (j=0; j<montgomery_ulongs; j++) montgomery_modulo_R2[j]=h[j];
}
...
ulong montgomery_inverse()
{
  ...
  if (montgomery_modulo_n[0]==1) return -1;
  v1=0; v2=1;
  b=-montgomery_modulo_n[0];
  p=montgomery_modulo_n[0];  /* 2^32-b */ v1+=v2;
  if (b>=p) {
    q=b/p;
    v2+=q*v1;
    q*=p;
...
int set_montgomery_multiplication(mpz_t n)
{
  ...
  if (montgomery_ulongs<2) montgomery_ulongs=2; /* have no 32 bit functions */
...
zeitB(2);
  ...
  if (old!=montgomery_ulongs) {
    ...
    } else Schlendrian("set_montgomery_multiplication\n");
  }
#ifdef MPQS_ZEIT
zeitA(9);
#endif
  if (montgomery_ulongs==2) init_montgomery_R2_2();
  else if (montgomery_ulongs==3) init_montgomery_R2_3();
  else init_montgomery_R2();
