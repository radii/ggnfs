...
#ifdef MPQS_STAT
extern ull stat_asm_eval, stat_asm_td;
extern ulong stat_asm_div, stat_final_mulmod;
#endif
...
static ulong mpqs_multiplier, mpqs_nfactors;
...
static uchar mpqs_A_mask[(1<<MPQS_MAX_ADIV_ALL)];
static ushort mpqs_nA;
static short mpqs_A_index, mpqs_B_index;
static ushort mpqs_SI_add[MPQS_MAX_ADIV_ALL][MPQS_MAX_FBSIZE];
static ushort mpqs_2A_all_inv[MPQS_MAX_FBSIZE];
...
static ulong mpqs_disp;
static ushort mpqs_nsurvivors;
static ulong mpqs_nrels, mpqs_nlp, mpqs_nsp, mpqs_excess;
...
static ulong mpqs_hash_index[128][16];
static ulong mpqs_lp[128*15];
...
static ulong **mpqs_gauss_row;
static uchar mpqs_sol[MPQS_GAUSS_MAX];
static short mpqs_gauss_c[MPQS_GAUSS_MAX], mpqs_gauss_d[MPQS_GAUSS_MAX];
...
static ull mpqs_kN_64, mpqs_A_inv_64;

ushort mpqs_FB[2*MPQS_MAX_FBSIZE];
ushort mpqs_FB_start[2*MPQS_MAX_FBSIZE];
ulong mpqs_sievelen;
ulong mpqs_FB_inv[MPQS_MAX_FBSIZE];
uchar *mpqs_sievearray;
...
#ifdef MPQS_STAT
ulong stat_mpqs_nsieves, stat_mpqs_nsurvivors, stat_mpqs_ntrials, stat_mpqs_ndiv;
#endif



static ull mpqs_inv_64(ull a)
{
  ull inv, h;

  inv=(ull)mpqs_256_inv_table[(a&0xff)>>1];
  h=a*inv; h&=0xff00;
  h*=inv; inv-=h;
  h=a*inv; h&=0xffff0000;
  h*=inv; inv-=h;
  h=a*inv; h&=0xffffffff00000000;
  h*=inv; inv-=h;
...
static long mpqs_jacobi(ushort a, ushort b) /* always gcd(a,b)=1 and 0<a<b */
{
  long e, l, m, n, r;
...
static long mpqs_jacobi0(ushort a, ushort b) /* always gcd(a,b)=1 and 0<a<b */
{
  long e, l, m, n;
...
/*  if ((v2*a-1)%(ulong)p) complain("invert");*/
  return v2;
}
                 ...
                 /* a!=0, a<p, p prime, p!=2, (a/p)=1 */
{
  ...
  while (k!=1) {
    b=k;
    for (i=0; i<e && b!=1; i++) { b*=b; b%=(ulong)p; }
    for (l=i+1; l<e; l++) { g*=g; g%=(ulong)p; }
    r*=g; r%=(ulong)p;
    g*=g; g%=(ulong)p;
    k*=g; k%=(ulong)p;
    e=i;
...
#if 0
static ulong mpqs_inv(ushort i)
{
  ulong v1=0, v2=1, q, a, p;
...
#else
static ulong mpqs_inv(ushort a)
{
  ulong inv, h;

  inv=(ulong)mpqs_256_inv_table[(a&0xff)>>1];
  h=(ulong)a*inv; h&=0xff00;
  h*=inv; inv-=h;
  h=(ulong)a*inv; h&=0xffff0000;
  h*=inv; inv-=h;
  if (inv*(ulong)a!=1ULL) Schlendrian("mpqs_inv");
  return inv;
}
#endif
...
void mpqs_init()
{
  ...
  mpqs_sievearray=(uchar *)xmalloc(MPQS_SIEVELEN*sizeof(uchar));
  ...
  mpqs_gauss_row=(ulong **)xmalloc(MPQS_GAUSS_MAX*sizeof(ulong *));
  mpqs_gauss_row[0]=(ulong *)xmalloc(MPQS_GAUSS_MAX*MPQS_GAUSS_MAX/32*sizeof(ulong));
#if 0
  prime_table_init();
  for (i=0; i<MPQS_MAX_NPRIMES; i++) {
    mpqs_prime_table[i]=(ushort)get_next_prime();
...
printf("%ld ",mpqs_multiplier);
  ...
  mpqs_kN_64=(ull)mpz_get_ui(mpqs_dummy); mpqs_kN_64<<=32;
  mpqs_kN_64+=(ull)mpz_get_ui(mpqs_kN);
}
...
static void mpqs_generate_FB()
{
  ...
  *fb++=2; *fb++=1; nfb++;
  for (i=1; i<MPQS_MAX_NPRIMES; i++) {
    ...
    rest=(ushort)mpz_mod_ui(mpqs_dummy,mpqs_kN,p);  /* ca 5M */
    if (rest) {
...
{
  long i, ind, j;
  long long add, bi, prod_other;
  ushort p, s1, s2, inv, *fb=mpqs_FB, *fbs=mpqs_FB_start;
  short sh;
  ulong bb, cc, c1, c2;
  uchar mask;
  double d;
...
/* compute zeros */
    ...
    if (d<(double)mpqs_disp) {
      mpqs_zero1=mpqs_disp-(ushort)d;
      mpqs_zero2=mpqs_disp+(ushort)d;
    } else {
...
/* compute B_i's */
    for (i=0; i<mpqs_nAdiv; i++) {
      ...
      inv=(ushort)(bi%(long long)p); /* bi>0 */
      inv=mpqs_invert(inv,p);
      bi*=(long long)inv; bi%=mpqs_A;
      bi*=(long long)mpqs_Adiv_sqrt[i]; bi%=mpqs_A;
      mpqs_Bi[i]=bi;
    }
    mpqs_B=0;
    ...
    prod_other=1;
    for (i=0; i<mpqs_nAdiv_total; i++)
      if (!mpqs_Adiv_active[i]) prod_other*=(long long)(mpqs_Adiv_all[i]);
    for (i=1; i<mpqs_nFB; i++) {
      p=fb[2*i];
#if 0
      bb=(ulong)(prod_other%(long long)p);
      bb*=(ulong)(mpqs_2A_all_inv[i]); bb%=(ulong)p;
#else
bb=(ulong)(((long long)(mpqs_2A_all_inv[i])*prod_other)%(ulong)p);
#endif
      for (j=0; j<mpqs_nAdiv; j++) {
#if 1
        cc=bb*(ulong)(mpqs_Bi[j]%(long long)p); cc%=(ulong)p;
#else
cc=(ulong)(((long long)bb*mpqs_Bi[j])%(ulong)p);
#endif
        mpqs_SI_add[j][i]=(ushort)cc;
      }
      if (bb&1) bb+=p;
      bb>>=1;  /* now bb=1/A mod p */
      cc=p-(ulong)(mpqs_B%(long long)p);  /* mpqs_B>0 */
      c1=cc+fb[2*i+1]; c1*=bb; c1+=mpqs_disp; c1%=(ulong)p;
      c2=cc+(p-fb[2*i+1]); c2*=bb; c2+=mpqs_disp; c2%=(ulong)p;
#if 0
      if (c1<c2) { fbs[2*i]=(ushort)c1; fbs[2*i+1]=(ushort)c2; }
      else { fbs[2*i]=(ushort)c2; fbs[2*i+1]=(ushort)c1; }
#else
      fbs[2*i]=(ushort)c1; fbs[2*i+1]=(ushort)c2;
#endif
    }
    ...
    for (i=0; i<mpqs_nAdiv_total; i++)
      if (!mpqs_Adiv_active[i]) {
        ...
        bb>>=1;  /* now bb=1/A mod p */
        cc=p-(ulong)(mpqs_B%(long long)p);  /* mpqs_B>0 */
        c1=cc+mpqs_Adiv_all_sqrt[i]; c1*=bb; c1+=mpqs_disp; c1%=(ulong)p;
        c2=cc+(p-mpqs_Adiv_all_sqrt[i]); c2*=bb; c2+=mpqs_disp; c2%=(ulong)p;
        if (c1<c2) {
          mpqs_Adiv_start1[i]=(ushort)c1; mpqs_Adiv_start2[i]=(ushort)c2;
        } else {
          mpqs_Adiv_start1[i]=(ushort)c2; mpqs_Adiv_start2[i]=(ushort)c1;
        }
      } else {
        p=mpqs_Adiv_all[i];
        sh=(short)(mpqs_2B%(long long)p);
        if (sh>0) bb=(ulong)sh; else bb=(ulong)((short)p+sh);
        bb=(ulong)mpqs_invert(bb,p);
        sh=(short)(mpqs_C%(long long)p); sh=-sh;
        if (sh>0) cc=(ulong)sh; else cc=(ulong)((short)p+sh);
        cc*=bb; cc%=(ulong)p;  /* now p|2*B*cc+C */
        cc+=mpqs_disp; cc%=(ulong)p;
...
#else
      p=fb[2*j]; /*s1=fbs[2*j]; s2=fbs[2*j+1];*/
      aa=mpqs_SI_add[i][j]; al=(((ulong)aa)<<16)+(ulong)aa;
      ul=(ulong *)(fbs+2*j);
      *ul+=al;
...
#else
      p=fb[2*j]; /*s1=fbs[2*j]; s2=fbs[2*j+1];*/
      aa=p-mpqs_SI_add[i][j]; al=(((ulong)aa)<<16)+(ulong)aa;
      ul=(ulong *)(fbs+2*j);
      *ul+=al;
/*      fbs[2*j]+=aa; fbs[2*j+1]+=aa;*/
      if (fbs[2*j]>=p) fbs[2*j]-=p;
...
static void mpqs_sieve()
{
  ...
  long i;
  uchar lo;
  ulong *ulsv, *ulsvend, mask;

  ulsv=(ulong *)mpqs_sievearray;
  ulsvend=ulsv+mpqs_sievelen/4;
  if (mpqs_B&1) {
    mask=(ulong)(mpqs_FB_log[0]); mask*=0x00040004;
  } else {
    mask=(ulong)(mpqs_FB_log[0]); mask*=0x04000400;
  }
  while (ulsv<ulsvend) {
    *ulsv++=mask;
    *ulsv++=mask;
...
/*printf("%lu %lu \n",(ulong)(mpqs_sievearray[0]),(ulong)(mpqs_sievearray[1]));*/
/*  mask*=0x00000101;
  if (mpqs_zero1<mpqs_sievelen) {
    s1=mpqs_zero1/4; if (s1<128) s1=0; else s1-=128;
    ulsv=(ulong *)mpqs_sievearray; ulsv+=s1; ulsvend=ulsv+256;
...
static inline ushort mpqs_hash_lp(ulong lp)
{
  long j;
  ulong lp1;
  ushort lphash, nh;
...
static void mpqs_decompose()
{
  ...
  double dbl_qx, dx;
  short x;
  ulong *ultarg, *ulsrc, lp, ulqx;
  ulong inv, h, hi, ls1, ls2;
  ulong ax, ay, az, at, asmh;
  ulong rr, jj;
...
/*      for (j=0; j<mpqs_nFBk; j++) {
    ...
    pr=1;
    for (j=0; j<nr; j++) pr*=(ull)(mpqs_FB[2*(rels[i][5+j]-mpqs_nFBk_1)]);
    rels[i][4]=nr;
    if (minus) {
...
zeitB(12);
    ...
    ax=(ulong)qx; ay=(ulong)pr; /*az=0;*/
    inv=(ulong)mpqs_256_inv_table[(ay&0xff)>>1];
...
ulqx=az;
 ...
 zeitB(12);
    ...
    for (j=0; j<nr; j++) {
      ...
      while (1) {
        rr=ulqx*inv;
        if (((ull)rr*(ull)p) & 0xffffffff00000000ULL) {
/*          if (ulqx%(ulong)p==0) complain("%lu %lu %lu %lu y",rr,p,ulqx,inv);*/
          break;
        }
/*        if (ulqx%(ulong)p) complain("z");*/
        if (rels[i][4]<MPQS_TD_MAX_NDIV) {
          rels[i][5+rels[i][4]]=ii; rels[i][4]++;
 /*         ulqx/=(ulong)p;*/
          ulqx=rr;
...
static void mpqs_matrix_init()
{
  ...
  ulong mask;
#ifdef PRUNING
  ushort mpqs_gauss_wt[300];  /* verbessern !!!! */
...
stat_final_mulmod++;
    ...
    for (j=0; j<mpqs_nrels; j++)
      if (mpqs_sol[j]) {
        up=(ull *)(mpqs_relations[j]); /*printf("Q: %Lu  ",*up);*/
    ...
    for (j=0; j<mpqs_ncrels; j++)
      if (mpqs_sol[mpqs_nrels+j]) {
        up=(ull *)(mpqs_comb_rels[j]); /*printf("Q: %Lu  ",*up);*/
        mpz_set_ull(mpqs_gmp_help,*up);
        mpz_mul(mpqs_sq2,mpqs_sq2,mpqs_gmp_help);
        mpz_mod(mpqs_sq2,mpqs_sq2,mpqs_N);
        up++;
...
{
#ifdef TOTAL_STAT
  printf("%u -> %u trial division candidates, %u treated by fp tds\n",
	 tdc_total,tdc_total-tdc_discard,nc52);
