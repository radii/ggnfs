...
int psp64()
{
  ...
  if (!(montgomery_modulo_n[0]&1)) return 0;

  ex[0]=montgomery_modulo_n[0]-1; ex[1]=montgomery_modulo_n[1];
  if (!(ex[0]|ex[1])) return 0;
  e=0;
  while (!(ex[0]&1)) {
    ex[0]=(ex[0]>>1)|(ex[1]<<31); ex[1]>>=1;
    e++;
  }
  x[0]=1; x[1]=0;
  ...
  one[0]=x[0]; one[1]=x[1];
  if (ex[1]) {
    for (i=0; i<32; i++) if (ex[1]&(1<<i)) b=i;
    b+=32;
  } else {
    for (i=0; i<32; i++) if (ex[0]&(1<<i)) b=i;
  }
  s=1;
  for (i=b-1; i>=0; i--) {
    if (s>=32) break;
    s<<=1;
    if (i>=32) v=ex[1]&(1<<(i-32));
    else v=ex[0]&(1<<i);
    if (v) s++;
  }
  x[0]=0; x[1]=0;
  ...
  for (; i>=0; i--) {
    asm_sqm64(x,x);
    if (i>=32) v=ex[1]&(1<<(i-32)); else v=ex[0]&(1<<i);
    if (v) asm_add64(x,x);
  }  /* x=2^ex */
  if (asm_cmp64(x,one)==0) return 1;
...
int psp(mpz_t n)
{
  ...
  if (montgomery_ulongs==2) return psp64();
  ...
  e=0;
  while (!(ex[0]&1)) {
    for (i=0; i<montgomery_ulongs-1; i++) ex[i]=(ex[i]>>1)|(ex[i+1]<<31);
  ...
  s=1;
  for (i=b-1; i>=0; i--) {
    if (s>=16*montgomery_ulongs) break;
    s<<=1;
    v=ex[i>>5]&(1<<(i&31));
    if (v) s++;
  }
