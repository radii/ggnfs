...
$Id: batch_factor.c,v 1.1 2007-12-16 03:54:47 jasonp_sf Exp $
----------------------------------------------------------------------*/
...
static void check_relation(msieve_obj *obj,
				relation_batch_t *rb,
				uint32 index,
				mp_t *prime_product) {
	...
	rb->num_success++;
	print_relation(obj, c->a, c->b,
			f, c->num_factors_r, lp_r,
			f + c->num_factors_r, c->num_factors_a, lp_a);
}

/*------------------------------------------------------------------*/
static void compute_remainder_tree(msieve_obj *obj,
	...
	if (numerator->nwords <= MAX_MP_WORDS) {
		...
		while (first <= last)
			check_relation(obj, rb, first++, &num);
		return;
	}
	...
	compute_remainder_tree(obj, first, mid, rb, &remainder);
	compute_remainder_tree(obj, mid + 1, last, rb, &remainder);
...
uint32 relation_batch_run(msieve_obj *obj, relation_batch_t *rb) {
	...
	if (rb->num_relations > 0) {
		compute_remainder_tree(obj, 0, rb->num_relations - 1,
					rb, &rb->prime_product);
	}
