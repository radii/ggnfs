...
$Id: ffpoly.c,v 1.1 2007-12-16 03:54:46 jasonp_sf Exp $
----------------------------------------------------------------------*/
...
#error "factor base generation requires poly degree 6 or less"
#endif
...
static INLINE uint32 mul_mac(uint32 a, uint32 b, uint32 c, 
	...
	/* For 32-bit a, b, q, c, _mod and 64-bit psq compute
	   ...
	   performance will be dismal. */
		...
		defined(__ICL)) && defined(__i386__)
...
uint32 inv_sqrt_mod_q(mp_poly_t *res, mp_poly_t *s_in, mp_poly_t *f_in,
	...
	if (i == NUM_ISQRT_RETRIES && q < 150) {
		uint32 c0, c1, c2, c3 = 0, c4 = 0, c5 = 0;
