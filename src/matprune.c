...
/* Copyright 2006, Anton Korobeynikov.                        */
/**************************************************************/
...
"|        This is the matprune program for GGNFS.           |\n"\
"| Version: %-25s                       |\n"\
...
{ s32 i, maxDataSize, maxCols;
  ...
  if (ll_init(C, maxDataSize, maxCols)) {
    fprintf(stderr, "ll_init() failed in cm_init! Terminating... \n");
    exit(-1);    
  }
...
{ llist_t C;
  ...
  retval = checkMat(&M, delCols, &numDel);
  if (retval > 2) {
...
/****************************************************/
{
  ...
  msgLog("", "GGNFS-%s : matprune", GGNFS_VERSION);
  
  startTime = sTime();

  /* Build and prune the sparse matrix. */
  retval = buildSparseMat(colName, wtFactor);

  if (retval >0) {
    stopTime = sTime();
    printf("Total elapsed time: %1.2lf seconds.\n", stopTime-startTime);

    printf("Matrix is pruned. You can now run matsolve.\n");
    msgLog("", "Matrix is pruned. Run matsolve.");

    msgLog("", "Heap stats for matprune run:");
    logHeapStats();
  }
